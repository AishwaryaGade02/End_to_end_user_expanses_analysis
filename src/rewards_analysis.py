...
class RewardsOptimizer:
    ...
    def __init__(self, cards_df, transactions_df, mcc_df):
        ...
        self.card_rewards_database = {
            'Chase Freedom Unlimited': {
                'annual_fee': 0,
                'categories': {
                    'Eating Places and Restaurants': 0.03,  # 3% on dining
                    'Drug Stores and Pharmacies': 0.03,     # 3% on drugstores
                    'Travel - Chase Portal': 0.05,          # 5% on Chase Travel
                    'default': 0.015                        # FIXED: 1.5% on everything else
                },
                'signup_bonus': 200,
            ...
            'Chase Sapphire Preferred': {
                'annual_fee': 95,
                'categories': {
                    'Eating Places and Restaurants': 0.03,     # FIXED: 3x points on dining
                    'Travel - General': 0.02,                 # 2x on travel
                    'Travel - Chase Portal': 0.05,            # 5x on Chase Travel
                    'Online Grocery': 0.03,                   # 3x on online grocery (excluding Target, Walmart)
                    'Streaming Services': 0.03,               # 3x on select streaming
                    'Lyft': 0.05,                            # 5x on Lyft (through Sept 2027)
                    'default': 0.01                           # 1x on everything else
                },
                'signup_bonus': 600,
                'spending_requirement': 5000,
                'transfer_partners': True,
                'hotel_credit': 50,  # Annual $50 hotel credit
                'anniversary_bonus': 0.10  # 10% anniversary bonus
            },
            'American Express Gold': {
                'annual_fee': 250,
                'effective_annual_fee': 130,  # After $120 dining credit
                'categories': {
                    'Eating Places and Restaurants': 0.04,     # 4x points on dining (worldwide)
                    'Grocery Stores, Supermarkets': 0.04,     # 4x on groceries (up to $25k/year cap)
                    'default': 0.01                           # 1x on everything else
                },
                'signup_bonus': 600,  # Up to 90,000 points
                'spending_requirement': 4000,
                'credits': 120,  # Annual dining credit
                'spending_caps': {
                    'Grocery Stores, Supermarkets': 25000  # $25k annual cap on 4x grocery
                }
            },
            'Citi Double Cash': {
                'annual_fee': 0,
                'categories': {
                    'default': 0.02  # Simple 2% on everything (1% earn + 1% pay)
                },
                'signup_bonus': 200,
            ...
            'Capital One Savor': {
                'annual_fee': 95,
                'categories': {
                    'Eating Places and Restaurants': 0.04,        # 4% on dining
                    'Amusement Parks, Carnivals, Circuses': 0.04, # 4% on entertainment 
                    'Grocery Stores, Supermarkets': 0.02,         # 2% on groceries
                    'Streaming Services': 0.04,                   # 4% on streaming
            ...
            'Discover it Cash Back': {
                'annual_fee': 0,
                'categories': {
                    # CORRECTED: Rotating categories with proper caps
                    'Rotating Category Q1': {'rate': 0.05, 'quarterly_cap': 1500},  # 5% up to $1500/quarter
                    'Rotating Category Q2': {'rate': 0.05, 'quarterly_cap': 1500},  # Amazon, Streaming (Q2 2025)
                    'Rotating Category Q3': {'rate': 0.05, 'quarterly_cap': 1500},  # Gas, Entertainment (typical)
                    'Rotating Category Q4': {'rate': 0.05, 'quarterly_cap': 1500},  # PayPal, Wholesale (typical)
                    'default': 0.01
                },
                'signup_bonus': 0,
                'cashback_match': True,  # First year cashback matched (effectively doubles rewards)
                'rotating_categories': True
            },
            # Additional popular cards for better comparison
            'Chase Sapphire Reserve': {
                'annual_fee': 550,
                'effective_annual_fee': 250,  # After $300 travel credit
                'categories': {
                    'Eating Places and Restaurants': 0.03,     # 3x on dining
                    'Travel - General': 0.03,                 # 3x on travel
                    'Travel - Chase Portal': 0.05,            # 5x on Chase Travel (10x total with 1.5x redemption)
                    'Lyft': 0.05,                            # 5x on Lyft
                    'default': 0.01
                },
                'signup_bonus': 600,
                'spending_requirement': 5000,
                'transfer_partners': True,
                'travel_credit': 300,  # Annual $300 travel credit
                'priority_pass': True,
                'redemption_bonus': 1.5  # 50% bonus when redeeming through Chase Travel
            },
            'Blue Cash Preferred from American Express': {
                'annual_fee': 95,
                'categories': {
                    'Grocery Stores, Supermarkets': 0.06,      # 6% on groceries (up to $6k/year)
                    'Streaming Services': 0.06,                # 6% on streaming
                    'Gas Stations': 0.03,                     # 3% on gas
                    'default': 0.01
                },
                'signup_bonus': 300,
                'spending_requirement': 3000,
                'spending_caps': {
                    'Grocery Stores, Supermarkets': 6000   # $6k annual cap on 6% grocery
                }
            }
        }
    ...
    def _get_current_portfolio(self, user_id):
        ...
        user_cards = self.cards_df[(self.cards_df['client_id'] == user_id) & (self.cards_df['card_type'] == 'Credit')]
